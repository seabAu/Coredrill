...
The Phase 0 tables remain compatible and continue to store the durable capture envelope before acknowledgement. Versions 3–31 implement the reviewed tracker, pipeline, saved-view, and document-manifest persistence without promoting an Inbox receipt into a job, seeding provisional display vocabulary, storing attachment bytes in SQLite, or introducing an AI dependency. Broader indexes/FTS, tombstones, and binary attachment I/O remain owned by later checklist slices.
