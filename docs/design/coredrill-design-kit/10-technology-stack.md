...
**Phase 0 evidence (2026-08-24):** `EXT-001` through `EXT-003` build the Chromium Manifest V3 side panel and popup with WXT `0.21.4`/Vite `8.2.2`, inspect an exact `activeTab`/`scripting`/`storage` plus generated `sidePanel` manifest with no host permissions, exercise a synthetic user-action JobPosting/selection fixture, and prove the bounded checksummed local outbox. WXT `0.20.27` was rejected after its obsolete browser-runner subtree failed the advisory gate; the final 459-entry npm graph is clean. This does not yet accept `D-023`: transfer, Firefox/manual fallback, hostile-message/replay tests, final production inspection, and the `EXT-008` decision remain required. See [extension capture/outbox verification](../../proof/extension-capture-outbox-verification.md).
