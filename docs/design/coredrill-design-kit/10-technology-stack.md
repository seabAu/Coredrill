...
| Editor | Tiptap open-source core 3.30.5 + selected extensions | Accepted restricted schema and versioned Coredrill IR; no cloud/collaboration dependency |
...
- Native SQLite through the accepted narrow first-party Tauri/`rusqlite` command layer in [ADR-0004](../../adr/0004-adopt-tauri-rusqlite-native-boundary.md); the official SQL plugin is not selected.
- Preserve the shared transaction, migration, backup, parameter-binding, confinement, recovery, and cross-platform contracts at that boundary.
...
`NAT-001` through `NAT-008` pin Tauri 2.11.3/CLI 2.11.3 and accept `rusqlite` 0.40.1 for the native database boundary. The shell builds the shared Vite frontend with exact custom-command permissions; the real-process adapter passes the shared callback-transaction and migration/repository contracts; and the pinned Tauri application-data resolver feeds canonical, link-confined database and attachment roots. Secure storage uses exact `keyring-core` 1.0.0, `windows-native-keyring-store` 1.1.0, `apple-native-keyring-store` 1.0.2 with `keychain`, `zbus-secret-service-keyring-store` 1.0.1 with `crypto-rust`, and `zeroize` 1.9.0 through target-confined dependencies.
...
**Phase 0 evidence (2026-08-24):** `EXT-001` through `EXT-008` accept the WXT `0.21.4`/Vite `8.2.2` Manifest V3 baseline after exact Chromium and Firefox side-panel/sidebar plus popup builds, synthetic user-action capture, bounded checksummed outbox, durable SQLite-before-ack transfer, attempt-2 retry, exact deduplication, and checksummed Firefox export/import proof. Both unpacked targets and byte-identical store ZIPs pass exact manifest/file/CSP, remote-code, and secret inspection. The 45-file Firefox source-review ZIP cleanly rebuilds the exact production directory, store ZIP, and source ZIP from the frozen lockfile. Hostile sender/message, wrong-origin/ID, checksum, expiry, oversize, replay, and arbitrary-fetch-shaped cases fail closed. WXT `0.20.27` was rejected after its obsolete vulnerable runner subtree failed the advisory gate; the final 459-entry npm graph is clean. Clean-commit [Foundation CI run 32764058550](https://github.com/seabAu/Coredrill/actions/runs/32764058550) and its downloaded immutable artifact complete `D-023`; see [ADR-0005](../../adr/0005-adopt-wxt-multisurface-extension-baseline.md), [capture/outbox verification](../../proof/extension-capture-outbox-verification.md), [transfer/fallback/security verification](../../proof/extension-transfer-fallback-security-verification.md), and [production-package verification](../../proof/extension-production-package-verification.md).
...
| Rich editing | Tiptap 3.30.5 JSON as editable adapter representation; canonical content is Coredrill document IR 1 |
...
**Phase 0 evidence (2026-08-24; security maintenance 2026-09-03 and 2026-09-17):** `EDT-001` through `EDT-006` accept this exact local document baseline after canonical-schema, round-trip, hostile-paste, keyboard semantics, 100-page stress, source-mapped DOCX/PDF/text import, scanned-file, controlled DOCX, tagged PDF, rendered pagination, advisory, and license proof. See [ADR-0006](../../adr/0006-adopt-tiptap-local-document-baseline.md) and [document editor/export verification](../../proof/document-editor-export-verification.md). [ADR-0007](../../adr/0007-patch-tiptap-prototype-manipulation.md) and [ADR-0008](../../adr/0008-patch-tiptap-markdown-redos.md) successively supersede only the exact Tiptap package pin, now 3.30.5; no document or runtime boundary changes.
...
`XTR-008` adds no runtime dependency. A dependency-free ESM scoring module and
Vitest edge-case suite consume a checked-in version-1 evaluation manifest. A Node
generator loads the current `@coredrill/extractors` build, uses the existing
test-only LinkeDOM boundary for generic-document fixtures, and writes a
deterministic JSON report covering exact errors, precision, coverage/recall, and
confidence calibration at overall, exact adapter/version, and supported-field
levels. Root verification rebuilds the relevant packages and rejects a missing or
stale report. Threshold failures, absent samples, and insufficient samples remain
machine-readable; report generation does not require network access, AI, product
telemetry, SQLite, or a hosted service.
