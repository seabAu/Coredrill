...
**Phase 0 evidence (2026-08-24):** `EXT-001` through `EXT-006` build exact Chromium and Firefox Manifest V3 targets with WXT `0.21.4`/Vite `8.2.2`, retain no host/content-script permissions, exercise a synthetic user-action capture, and prove the bounded checksummed outbox locally and on clean-commit hosted CI. Chromium permits only the reserved Phase 0 HTTPS app origin; a real persistent-context E2E proves durable SQLite-before-ack receipt, acknowledgement loss, attempt-2 retry, exact deduplication, and queue removal. Firefox declares no data collection and proves the explicit checksummed JSON export/import fallback in a real browser. Hostile sender/message, wrong-origin/ID, checksum, expiry, oversize, replay, and arbitrary-fetch-shaped cases fail closed. WXT `0.20.27` was rejected after its obsolete vulnerable runner subtree failed the advisory gate; the final 459-entry npm graph is clean. This does not yet accept `D-023`: the final production/store-package secret review and `EXT-008` decision remain required. See [capture/outbox verification](../../proof/extension-capture-outbox-verification.md) and [transfer/fallback/security verification](../../proof/extension-transfer-fallback-security-verification.md).
