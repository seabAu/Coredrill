...
- Provider secrets use an OS keyring/secure-storage plugin; never the SQLite database or repository config in plaintext.

`NAT-001` through `NAT-004` pin Tauri 2.11.3/CLI 2.11.3 and carry `rusqlite` 0.40.1 as the provisional candidate. The Windows shell builds the shared Vite frontend with one capability-gated command; the real-process native adapter passes the shared callback-transaction and migration/repository contracts; and the exact pinned Tauri application-data resolver feeds canonical, link-confined database and content-addressed attachment roots. The official SQL plugin remains a live alternative because this checkpoint does not yet prove backup/atomic replacement, secure storage, installable packaging, or cross-platform behavior. The exact matrix and reviewed Cargo risk are in [native verification](../../proof/native-sqlite-tauri-verification.md); no Accepted decision changes here.
