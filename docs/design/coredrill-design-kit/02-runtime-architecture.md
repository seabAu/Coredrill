...
The adapter choice is not yet Accepted. `NAT-004` platform-tests real Tauri app-data resolution, missing/unusable roots, content-addressed attachment layout, and symlink/Windows-junction rejection; `NAT-005` proves the redacted Windows secure-store lifecycle; and `NAT-006` proves native export/restore recovery. `NAT-007` and `NAT-008` must still prove installable packaging, resource behavior, cross-platform dependencies, and the final adapter decision before D-022/Q-003 changes; see [native verification](../../proof/native-sqlite-tauri-verification.md), [secure-storage verification](../../proof/native-secure-storage-verification.md), and [native archive verification](../../proof/native-archive-verification.md).
