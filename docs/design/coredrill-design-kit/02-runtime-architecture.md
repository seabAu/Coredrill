...
The adapter choice is not yet Accepted. `NAT-004` platform-tests real Tauri app-data resolution, missing/unusable roots, content-addressed attachment layout, and symlink/Windows-junction rejection; `NAT-005` proves the redacted Windows secure-store lifecycle; `NAT-006` proves native export/restore recovery; and `NAT-007` proves first-OS packaging/resource behavior. `NAT-008` must still prove cross-platform dependencies and secure-store/package behavior, then record the final adapter decision before D-022/Q-003 changes; see [native verification](../../proof/native-sqlite-tauri-verification.md), [secure-storage verification](../../proof/native-secure-storage-verification.md), [native archive verification](../../proof/native-archive-verification.md), and [native package verification](../../proof/native-windows-package-verification.md).
