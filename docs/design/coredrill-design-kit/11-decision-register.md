...
- **Phase 0 evidence (2026-08-24):** `EXT-001` through `EXT-006` locally prove exact Chromium/Firefox WXT `0.21.4` MV3 side-panel/sidebar and popup builds, user-action capture, bounded checksummed outbox, durable SQLite-before-ack transfer, attempt-2 idempotent retry, Firefox checksummed JSON fallback, and hostile sender/message/oversize/wrong-origin-or-ID/expiry/replay rejection. Chromium has one reserved Phase 0 HTTPS external origin; Firefox has none and declares no data collection. The dependency gate rejected WXT `0.20.27` rather than suppressing its obsolete vulnerable runner subtree. Status remains Provisional pending clean-commit hosted proof, `EXT-007` final bundle/secret review, and `EXT-008`. See [capture/outbox verification](../../proof/extension-capture-outbox-verification.md) and [transfer/fallback/security verification](../../proof/extension-transfer-fallback-security-verification.md).
