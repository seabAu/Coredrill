...
- **Status:** Accepted
- **Decision:** Use Tiptap open-source core 3.30.5 behind Coredrill's restricted schema and version-1 canonical document IR; use locally owned Mammoth/PDF.js/text proposal importers, `docx` 9.7.1 for controlled DOCX output, and semantic browser/Tauri print HTML/CSS for tagged PDF output.
...
- **Security maintenance (2026-09-03):** [ADR-0007](../../adr/0007-patch-tiptap-prototype-manipulation.md) supersedes only the exact 3.30.2 package pin with aligned 3.30.4 packages, the first release patched for `GHSA-cp6q-959q-f8rh`; the accepted editor and document boundary is unchanged.
- **Security maintenance (2026-09-17):** [ADR-0008](../../adr/0008-patch-tiptap-markdown-redos.md) supersedes only the exact 3.30.4 package pin with aligned 3.30.5 packages, the first release patched for `GHSA-j95f-988m-3j2f`; Markdown ingestion remains disabled and the accepted editor/document boundary is unchanged.
...
| Q-004 | Tiptap suitability | [ADR-0006](../../adr/0006-adopt-tiptap-local-document-baseline.md) accepts restricted Tiptap behind canonical document IR 1 after round-trip, sanitation, stress, local import/export, rendered pagination, keyboard semantics, advisory, and license proof. [ADR-0007](../../adr/0007-patch-tiptap-prototype-manipulation.md) and [ADR-0008](../../adr/0008-patch-tiptap-markdown-redos.md) successively supersede only its exact package pin, now aligned 3.30.5, to resolve `GHSA-cp6q-959q-f8rh` and `GHSA-j95f-988m-3j2f`. | 2026-09-17 |
