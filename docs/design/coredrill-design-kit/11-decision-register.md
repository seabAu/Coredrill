...
- **Status:** Accepted
- **Decision:** Use Tiptap open-source core 3.30.4 behind Coredrill's restricted schema and version-1 canonical document IR; use locally owned Mammoth/PDF.js/text proposal importers, `docx` 9.7.1 for controlled DOCX output, and semantic browser/Tauri print HTML/CSS for tagged PDF output.
...
| Q-004 | Tiptap suitability | [ADR-0006](../../adr/0006-adopt-tiptap-local-document-baseline.md) accepts restricted Tiptap behind canonical document IR 1 after round-trip, sanitation, stress, local import/export, rendered pagination, keyboard semantics, advisory, and license proof. [ADR-0007](../../adr/0007-patch-tiptap-prototype-manipulation.md) supersedes only its exact package pin with aligned 3.30.4 packages to resolve `GHSA-cp6q-959q-f8rh`. | 2026-09-03 |
