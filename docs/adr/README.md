...
Provisional decisions `D-010`, `D-012`, `D-015`, `D-023`, and `D-027` retain their named Phase 0/usability gates. ADR-0003 promotes D-025 after exact hosted browser lanes passed and resolves Q-002 with an explicit unsupported-platform fallback. ADR-0004 accepts the Tauri 2 plus narrow `rusqlite` native boundary, resolves Q-003, and retains the local browser app plus portable export as the Linux fallback while native GTK3 risk remains unresolved. Deferred `D-052` does not authorize a sync service or account dependency.
