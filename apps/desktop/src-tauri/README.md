...
Provider secrets use one exact, target-confined operating-system backend: Windows Credential Manager, the macOS login Keychain, or FreeDesktop Secret Service. Every lifecycle request is scoped by durable vault ID plus provider ID. Each lifecycle is serialized; unavailable or locked secure storage fails closed with a stable content-free error, and there is no plaintext fallback. The redacted proof harness retrieves its synthetic value only inside Rust, zeroizes Coredrill-owned buffers, deletes the entry, and emits booleans plus the backend name rather than secret material.

The combined deletion command requires a preview-bound exact phrase before it
stages the target database, unshared attachment bytes, and target backup
directory. It removes only the target vault's registered OS-secret accounts,
restores staged content on failure, preserves shared/other-vault/external data,
and reports an honest cleanup-pending state when a final purge must be retried
at startup. Paths and provider identifiers never cross the command response.
